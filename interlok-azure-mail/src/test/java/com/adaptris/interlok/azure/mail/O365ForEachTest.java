...
public class O365ForEachTest {

  private static final byte[] PAYLOAD_1 = "Bacon ipsum dolor amet ball tip pancetta chuck boudin leberkas, alcatra shoulder pig meatball ground round cupim frankfurter chicken andouille. Landjaeger alcatra pork belly rump drumstick beef ribs shoulder. Corned beef venison meatloaf ham hock doner pig salami burgdoggen cow tongue. Swine pork loin ham, kevin flank pig salami biltong shankle ball tip alcatra short ribs hamburger tongue. Buffalo meatloaf capicola, leberkas jowl biltong kevin drumstick pastrami andouille."
      .getBytes();
  private static final byte[] PAYLOAD_2 = "Cupcake ipsum dolor sit amet danish fruitcake candy. Icing jujubes powder sweet. Pie gingerbread bonbon dragee lollipop sesame snaps. Bonbon gummi bears danish caramels cupcake powder. Gingerbread bonbon croissant tootsie roll jelly pastry. Candy canes powder sweet biscuit fruitcake. Dragee biscuit brownie biscuit pudding jelly. Muffin tootsie roll tiramisu ice cream macaroon cupcake. Sugar plum powder gummi bears dessert wafer. Cheesecake lollipop tootsie roll dessert lollipop icing cheesecake."
      .getBytes();

  private O365ForEach forEach;
  ...
  public void testForEachWithPayloadMetadata() throws Exception {
    ...
    message.addPayload("cupcake", PAYLOAD_2);
    new MultiPayloadAdaptrisMessageWrapper(message).addPayloadMessageHeader("bacon", "key", "baconValue");
    new MultiPayloadAdaptrisMessageWrapper(message).addPayloadMessageHeader("cupcake", "key", "cupcakeValue");
    forEach.doService(message);
    assertEquals(Integer.valueOf(1), forEach.getThreadCount());
    verify(mock, times(2)).doService(any(AdaptrisMessage.class));
  }
  ...
  public void testNonCloneableMessage() throws Exception {
    forEach
    .doService(new MultiPayloadAdaptrisMessageImp("bacon", new GuidGenerator(), DefaultMessageFactory.getDefaultInstance(), PAYLOAD_1) {
      @Override
